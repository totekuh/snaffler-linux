...
DATA_ROOT = Path(__file__).parents[3] / "data" / "cloud_api_keys_name"
...
def test_cloud_api_keys_files_by_name_trigger_keep_cloud_api_keys_rule(filename, evaluator):
    """
    Cloud API key files matched by exact filename MUST trigger KeepCloudApiKeysByName
    ...
    assert (
               ...
               MatchAction.SNAFFLE,
           ) in hits, f"KeepCloudApiKeysByName not triggered. Got: {hits}"
