from pathlib import Path
from datetime import datetime

import pytest

from snaffler.classifiers.default_rules import get_default_rules
from snaffler.classifiers.evaluator import RuleEvaluator
from snaffler.analysis.model.file_context import FileContext
from snaffler.classifiers.rules import (
    EnumerationScope,
    MatchAction,
)

# -----------------------------
# paths
# -----------------------------

DATA_ROOT = Path(__file__).parents[3] / "data" / "password_files"

FILES = [
    "passwords.txt",
    "secret_notes.txt",
    "credential_dump.txt",
    "thycotic_export.txt",
    "cyberark_backup.txt",
]


@pytest.fixture(scope="session")
def evaluator():
    return RuleEvaluator(
        file_rules=get_default_rules(),
        content_rules=[],
        postmatch_rules=[],
    )


def make_ctx(path: Path) -> FileContext:
    return FileContext(
        unc_path=str(path),
        smb_path=str(path),
        name=path.name,
        ext=path.suffix.lower(),
        size=path.stat().st_size,
        modified=datetime.fromtimestamp(0),
    )


@pytest.mark.parametrize("filename", FILES)
def test_password_name_triggers_green_rule(filename, evaluator):
    """
    Files containing password/secret/credential/etc in NAME
    MUST trigger KeepNameContainsGreen and result in SNAFFLE.
    """
    path = DATA_ROOT / filename
    assert path.exists(), f"Missing test file: {path}"

    ctx = make_ctx(path)

    hits = []

    for rule in evaluator.file_rules:
        if rule.enumeration_scope != EnumerationScope.FILE_ENUMERATION:
            continue

        decision = evaluator.evaluate_file_rule(rule, ctx)
        if decision:
            hits.append((rule.rule_name, decision.action))

    # 1. Something matched
    assert hits, f"No rules matched {filename}"

    # 2. Correct rule matched
    assert (
               "KeepNameContainsGreen",
               MatchAction.SNAFFLE,
           ) in hits, f"Expected KeepNameContainsGreen not triggered. Got: {hits}"
