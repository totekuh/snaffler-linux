...
DATA_ROOT = Path(__file__).parents[3] / "data" / "defender"
...
def test_defender_config_files_by_name_trigger_keep_rule(filename, evaluator):
    """
    Microsoft Defender config files matched by exact filename
    MUST trigger KeepDefenderConfigByName and result in SNAFFLE with YELLOW triage.
    """
    ...
    assert (
        ...
        MatchAction.SNAFFLE,
    ) in hits, f"KeepDefenderConfigByName not triggered. Got: {hits}"
