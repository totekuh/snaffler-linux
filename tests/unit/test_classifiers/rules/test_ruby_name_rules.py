...
DATA_ROOT = Path(__file__).parents[3] / "data" / "ruby_name"
...
def test_ruby_files_by_name_trigger_keep_ruby_rule(filename, evaluator):
    """
    Ruby files matched by exact filename MUST trigger KeepRubyByName
    ...
    assert (
               "KeepRubyByName",
               MatchAction.SNAFFLE,
           ) in hits, f"KeepRubyByName not triggered. Got: {hits}"
