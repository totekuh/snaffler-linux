...
DATA_ROOT = Path(__file__).parents[3] / "data" / "config"
...
def test_config_files_by_name_trigger_keep_config_rule(filename, evaluator):
    """
    Config files matched by exact filename MUST trigger KeepConfigByName
    ...
    assert (
               "KeepConfigByName",
               MatchAction.SNAFFLE,
           ) in hits, f"KeepConfigByName not triggered. Got: {hits}"
