...
DATA_ROOT = Path(__file__).parents[3] / "data" / "network_configs"
...
def test_network_config_files_by_name_trigger_keep_rule(filename, evaluator):
    """
    Network device config files matched by exact filename
    MUST trigger KeepNetConfigFileByName and result in SNAFFLE with BLACK triage.
    """
    ...
    assert (
        ...
        MatchAction.SNAFFLE,
    ) in hits, f"KeepNetConfigFileByName not triggered. Got: {hits}"