...
DATA_ROOT = Path(__file__).parents[3] / "data" / "password_managers"
...
def test_password_manager_files_by_extension_trigger_keep_pass_mgrs_rule(filename, evaluator):
    """
    Password manager files matched by extension MUST trigger KeepPassMgrsByExtension
    ...
    assert (
               ...
               MatchAction.SNAFFLE,
           ) in hits, f"KeepPassMgrsByExtension not triggered. Got: {hits}"
