...
DATA_ROOT = Path(__file__).parents[3] / "data" / "nix_hashes"
...
def test_nix_hash_files_trigger_black_rule(filename, evaluator):
    """
    Unix local hash files MUST trigger KeepNixLocalHashesByName
    ...
    assert (
               ...
               MatchAction.SNAFFLE,
           ) in hits, f"Expected KeepNixLocalHashesByName not triggered. Got: {hits}"
