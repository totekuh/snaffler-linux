...
DATA_ROOT = Path(__file__).parents[3] / "data" / "memory_dumps_name"
...
def test_memory_dump_files_by_name_trigger_keep_rule(filename, evaluator):
    """
    Critical memory dump files matched by exact filename
    MUST trigger KeepMemDumpByName and result in SNAFFLE with BLACK triage.
    """
    ...
    assert (
        "KeepMemDumpByName",
        MatchAction.SNAFFLE,
    ) in hits, f"KeepMemDumpByName not triggered. Got: {hits}"
