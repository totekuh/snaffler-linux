...
DATA_ROOT = Path(__file__).parents[3] / "data" / "kerberos_name"
...
def test_kerberos_credential_files_by_name_trigger_keep_rule(filename, evaluator):
    """
    Kerberos credential cache files matched by regex pattern (krb5cc_.*)
    MUST trigger KeepKerberosCredentialsByName and result in SNAFFLE with YELLOW triage.
    """
    ...
    assert (
        ...
        MatchAction.SNAFFLE,
    ) in hits, f"KeepKerberosCredentialsByName not triggered. Got: {hits}"
