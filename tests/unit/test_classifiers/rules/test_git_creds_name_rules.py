...
DATA_ROOT = Path(__file__).parents[3] / "data" / "git_creds_name"
...
def test_git_creds_files_by_name_trigger_keep_git_creds_rule(filename, evaluator):
    """
    Git credentials files matched by exact filename MUST trigger KeepGitCredsByName
    ...
    assert (
               "KeepGitCredsByName",
               MatchAction.SNAFFLE,
           ) in hits, f"KeepGitCredsByName not triggered. Got: {hits}"
