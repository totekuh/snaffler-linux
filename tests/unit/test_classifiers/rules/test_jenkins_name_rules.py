...
DATA_ROOT = Path(__file__).parents[3] / "data" / "jenkins_name"
...
def test_jenkins_files_by_name_trigger_keep_jenkins_rule(filename, evaluator):
    """
    Jenkins files matched by exact filename MUST trigger KeepJenkinsByName
    ...
    assert (
               "KeepJenkinsByName",
               MatchAction.SNAFFLE,
           ) in hits, f"KeepJenkinsByName not triggered. Got: {hits}"
