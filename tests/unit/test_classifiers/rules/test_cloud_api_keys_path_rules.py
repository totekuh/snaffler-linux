...
DATA_ROOT = Path(__file__).parents[3] / "data" / "cloud_api_keys_path"

# Files with paths that should match the rule
FILE_PATHS = [
    ".aws/credentials",
    ".aws/config",
    "doctl/config.yaml",
...
def test_cloud_api_keys_files_by_path_trigger_keep_cloud_api_keys_rule(file_path, evaluator):
    """
    Cloud API key files matched by path MUST trigger KeepCloudApiKeysByPath
    ...
    # something matched
    assert hits, f"No rules matched {file_path}"

    # correct rule fired
    assert (
               "KeepCloudApiKeysByPath",
               MatchAction.SNAFFLE,
           ) in hits, f"KeepCloudApiKeysByPath not triggered. Got: {hits}"
