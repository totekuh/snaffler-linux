...
DATA_ROOT = Path(__file__).parents[3] / "data" / "browser_credentials"
...
def test_browser_credential_files_trigger_relay_rule(filename, evaluator):
    """
    Browser credential files (logins.json) matched by exact filename MUST trigger
    KeepFfLoginsJsonRelay and result in RELAY.
    """
    ...
    # correct rule fired
    assert (
               "KeepFfLoginsJsonRelay",
               MatchAction.RELAY,
           ) in hits, f"KeepFfLoginsJsonRelay not triggered. Got: {hits}"