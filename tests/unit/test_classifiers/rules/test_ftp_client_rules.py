...
DATA_ROOT = Path(__file__).parents[3] / "data" / "ftp_client"
...
def test_ftp_client_config_files_by_name_trigger_keep_rule(filename, evaluator):
    """
    FTP client config files matched by exact filename
    MUST trigger KeepFtpClientConfigConfigByName and result in SNAFFLE with RED triage.
    """
    ...
    # correct rule fired
    assert (
        "KeepFtpClientConfigConfigByName",
        MatchAction.SNAFFLE,
    ) in hits, f"KeepFtpClientConfigConfigByName not triggered. Got: {hits}"
