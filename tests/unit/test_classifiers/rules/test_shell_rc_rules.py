...
DATA_ROOT = Path(__file__).parents[3] / "data" / "shell_rc"
...
def test_shell_rc_files_by_name_trigger_keep_shell_rc_rule(filename, evaluator):
    """
    Shell RC files matched by name MUST trigger KeepShellRcFilesByName
    ...
    assert (
               ...
               MatchAction.SNAFFLE,
           ) in hits, f"KeepShellRcFilesByName not triggered. Got: {hits}"
