...
DATA_ROOT = Path(__file__).parents[3] / "data" / "cyberark_extension"
...
def test_cyberark_files_by_extension_trigger_keep_rule(filename, extension, evaluator):
    """
    CyberArk files matched by extension (.cred, .pass)
    MUST trigger KeepCyberArkByExtension and result in SNAFFLE with RED triage.
    """
    ...
    assert (
        ...
        MatchAction.SNAFFLE,
    ) in hits, f"KeepCyberArkByExtension not triggered. Got: {hits}"
