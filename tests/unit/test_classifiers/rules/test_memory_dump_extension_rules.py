...
DATA_ROOT = Path(__file__).parents[3] / "data" / "memory_dumps_extension"
...
def test_memory_dump_files_by_extension_trigger_keep_rule(filename, extension, evaluator):
    """
    Memory dump files matched by extension (.dmp)
    MUST trigger KeepMemDumpByExtension and result in SNAFFLE with RED triage.
    """
    ...
    assert (
        ...
        MatchAction.SNAFFLE,
    ) in hits, f"KeepMemDumpByExtension not triggered. Got: {hits}"
