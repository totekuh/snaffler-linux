...
DATA_ROOT = Path(__file__).parents[3] / "data" / "ext_exact_yellow"
...
POSITIVE_CASES = [
    ("server.key", "KeepExtExactYellow"),
    ("app.keypair", "KeepExtExactYellow"),
    ("truststore.jks", "KeepExtExactYellow"),
]


@pytest.mark.parametrize("filename,expected_rule", POSITIVE_CASES)
def test_ext_exact_yellow_triggers(filename, expected_rule, evaluator):
