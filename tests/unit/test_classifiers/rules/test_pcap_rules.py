...
DATA_ROOT = Path(__file__).parents[3] / "data" / "pcap"

EXTENSION_FILENAMES = [
    ("network_traffic.pcap", ".pcap"),
    ("capture.cap", ".cap"),
    ("traffic.pcapng", ".pcapng"),
]
...
def test_pcap_files_by_extension_trigger_keep_rule(filename, extension, evaluator):
    """
    Packet capture files matched by extension (.pcap, .cap, .pcapng)
    MUST trigger KeepPcapByExtension and result in SNAFFLE with YELLOW triage.
    """
    ...
    assert (
        "KeepPcapByExtension",
        MatchAction.SNAFFLE,
    ) in hits, f"KeepPcapByExtension not triggered. Got: {hits}"
