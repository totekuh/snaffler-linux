...
DATA_ROOT = Path(__file__).parents[3] / "data" / "remote_access_name"
...
def test_remote_access_files_by_name_trigger_keep_remote_access_rule(filename, evaluator):
    """
    Remote access files matched by name MUST trigger KeepRemoteAccessConfByName
    ...
    assert (
               ...
               MatchAction.SNAFFLE,
           ) in hits, f"KeepRemoteAccessConfByName not triggered. Got: {hits}"
