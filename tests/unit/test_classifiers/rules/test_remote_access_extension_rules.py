...
DATA_ROOT = Path(__file__).parents[3] / "data" / "remote_access"
...
def test_remote_access_files_by_extension_trigger_keep_remote_access_rule(filename, evaluator):
    """
    Remote access files matched by extension MUST trigger KeepRemoteAccessConfByExtension
    ...
    assert (
               ...
               MatchAction.SNAFFLE,
           ) in hits, f"KeepRemoteAccessConfByExtension not triggered. Got: {hits}"
