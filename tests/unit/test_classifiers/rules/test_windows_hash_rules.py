...
DATA_ROOT = Path(__file__).parents[3] / "data" / "windows_hashes"
...
def test_windows_hash_files_trigger_black_rule(filename, evaluator):
    """
    Windows hash files MUST trigger KeepWinHashesByName
    ...
    assert (
               "KeepWinHashesByName",
               MatchAction.SNAFFLE,
           ) in hits, f"Expected KeepWinHashesByName not triggered. Got: {hits}"
