...
DATA_ROOT = Path(__file__).parents[3] / "data" / "deploy_images"

EXTENSION_FILENAMES = [
    ("windows10.wim", ".wim"),
    ("ubuntu-server.ova", ".ova"),
    ("centos-minimal.ovf", ".ovf"),
]
...
def test_deploy_image_files_by_extension_trigger_keep_rule(filename, extension, evaluator):
    """
    Deployment image files matched by extension (.wim, .ova, .ovf)
    MUST trigger KeepDeployImageByExtension and result in SNAFFLE with YELLOW triage.
    """
    ...
    assert (
        ...
        MatchAction.SNAFFLE,
    ) in hits, f"KeepDeployImageByExtension not triggered. Got: {hits}"
