...
DATA_ROOT = Path(__file__).parents[3] / "data" / "ftp_server"
...
def test_ftp_server_config_files_by_name_trigger_keep_rule(filename, evaluator):
    """
    FTP server config files matched by exact filename
    MUST trigger KeepFtpServerConfigByName and result in SNAFFLE with RED triage.
    """
    ...
    assert (
        ...
        MatchAction.SNAFFLE,
    ) in hits, f"KeepFtpServerConfigByName not triggered. Got: {hits}"
